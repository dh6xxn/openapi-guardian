...
function help() {
  console.log(`OpenAPI Guardian v0.4.0
...
<testsuite name="OpenAPI Guardian" tests="' + results.length + '" failures="' + failures.length + '">
' + cases + '
</testsuite>
';
}
...
OpenAPI Guardian
');
      for (const result of results) console.log((result.status === 'passed' ? '✔' : '✖') + ' ' + result.method.padEnd(7) + ' ' + result.path + (result.statusCode ? ' (' + result.statusCode + ')' : '') + (result.errors.length ? '
    ' + result.errors.join('
    ') : ''));
...
');
      ...
      for (const change of changes) console.log((change.type === 'breaking' ? '✖' : change.type === 'warning' ? '⚠' : '+') + ' ' + (change.method ? change.method.toUpperCase() + ' ' : '') + change.path + '
    ' + change.message);
